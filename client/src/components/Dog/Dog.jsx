...
export default function Dog({name,image,temperament,id}) { 
    return <div>
        ...
        {temperament ? temperament.map(d => <p className="temperamentsName" key={d.id}>{d.charAt(0).toUpperCase()+d.slice(1)}</p>) : "This dog no has temperament added"}
        </div>
        {/* <div className="link">
        <Link  to={`dogs/${id}`} > Read more</Link>
        </div> */}
        </div>
